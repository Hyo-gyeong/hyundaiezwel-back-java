...
public class ProductViewServlet extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.print("<td>ID</td><td>카데고리</td><td>상품명</td>"
				+ "<td>가격</td><td>삭제</td></tr>");
		for (int i = 0; i < prdList.size(); i++) {
			ProductVO vo = prdList.get(i);
			// 테이블 행 구성
			out.print("<tr><td>" + vo.getId() + "</td>");
			out.print("<td>" + vo.getCategory() + "</td>");
			out.print("<td>" + vo.getName() + "</td>");
			out.print("<td>" + vo.getPrice() + "</td>");
			out.print("<td><a href='/Servlet01/prdDelete?id=" +  vo.getId() + "'>삭제</a></td></tr>");
		}
