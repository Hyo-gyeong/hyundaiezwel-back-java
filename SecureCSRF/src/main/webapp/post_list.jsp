...
<body>
	<form action="post_read.jsp" method="get">
		<h1>게시글 목록</h1>
		<%
			// 로그인 증명 확인 위해 session 값을 조회(추출)
			String S_ID = (String) session.getAttribute("S_ID"); // 로그인된 사용자의 요청인지 확인
			String S_name = (String) session.getAttribute("S_name"); // 게시글 목록은 모든 사용자의 게시글을 볼 수 있기 때문에 S_name session 속성은 사용 안함
			if(S_ID != null){ // 로그인된 사용자만 목록 보기가 가능하도록 로그인되지 않은 경우 url통해 직접 접근도 금지
			try
			{
			...
			catch (Exception ex)
			{
				...
				response.sendRedirect("main.html"); // session 정보를 페이지 요청시 갖고 오지 않았으면 메인 페이지로 이동
			}%>
	</form>
</body>
</html>