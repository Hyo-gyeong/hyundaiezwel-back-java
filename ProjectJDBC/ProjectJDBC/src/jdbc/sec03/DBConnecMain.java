...
import java.sql.ResultSet; // 반환 결과가 릴레이션일 때 반환되는 결과를 저장하는 객체(select)
import java.sql.SQLException; // sql 질의에 해당하는 예외

public class DBConnecMain {
    public static void main(String[] args) {
        ...
        try {
            ...
            // rs는 2차원 배열의 시작점 앞을 포인팅하고 있는 포인터, 맨 마지막 행인 null을 포인팅할 때까지 반복
            // rs.next() : 현재 포인팅하는 튜플에서 다음 튜플로 포인터 이동, 다음 튜플이 있으면 true, 없으면 false
            // rs.getXXX(컬럼번호 또는 컬럼명) : 현재 포인팅하는 튜플의 컬럼 값 반환
            // XXX에는 타입이 들어감 ex. getString(), getInt(), getDouble(), getDate()
            while (rs.next()){
