...
public class UpdateMain {
    public static void main(String[] args) {
        ...
        sc.nextLine(); // nextInt에서 처리되지 못한 엔터값을 처리하려고
        ...
        try{
            ...
            pstmt = con.prepareStatement(sql); // 전처리 내부적으로 준비
            ...
            // 수정된 행의 수 정수로 반환 - 필요시 변수에 저장해서 사용
            // 질의 중 오류 발생하면 데이터베이스 오류(catch에서 해결)
            pstmt.executeUpdate(); // 이미 쿼리를 구성해서 바인딩까지 마쳤기 때문에 update만 실행하면 됨

            System.out.println("도서 정보 수정 성공!");
        }catch(SQLException e){
            System.out.println("도서 정보 수정 오류!");
            e.printStackTrace();
        }finally{
